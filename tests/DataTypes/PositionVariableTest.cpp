...
TEST(App_PositionVariableTest, ConstructorCartesian) {
  {
    ...
    App::DataTypes::PositionVariable programVariable(name, cartesianPos,
                                                     externalJoints);

    EXPECT_STREQ(name.c_str(), programVariable.GetName().c_str());
    for (size_t i = 0; i < 16; i++)
      EXPECT_EQ(cartesianPos[i], programVariable.GetCartesian()[i]);
    for (size_t i = 0; i < programVariable.GetRobotAxes().size(); i++)
      EXPECT_EQ(0, programVariable.GetRobotAxes()[i]);
...
TEST(App_PositionVariableTest, ConstructorJoints) {
  // name, default value
  {
    ...
    std::array<double, 3> externalJoints{0};
    App::DataTypes::PositionVariable programVariable(name, robotJoints,
                                                     externalJoints);

    App::DataTypes::Matrix44 cartesianExpect;
    for (size_t i = 0; i < 16; i++)
      EXPECT_EQ(cartesianExpect[i], programVariable.GetCartesian()[i]);
    for (size_t i = 0; i < programVariable.GetRobotAxes().size(); i++)
      EXPECT_EQ(0, programVariable.GetRobotAxes()[i]);
    EXPECT_EQ(externalJoints[0], programVariable.GetExternalAxes()[0]);
    EXPECT_EQ(externalJoints[1], programVariable.GetExternalAxes()[1]);
  ...
  // empty name, default value
  {
    ...
    for (size_t i = 0; i < 16; i++)
      EXPECT_EQ(cartesianExpect[i], programVariable.GetCartesian()[i]);
    for (size_t i = 0; i < programVariable.GetRobotAxes().size(); i++)
      EXPECT_EQ(robotJoints[i], programVariable.GetRobotAxes()[i]);
    EXPECT_EQ(externalJoints[0], programVariable.GetExternalAxes()[0]);
    EXPECT_EQ(externalJoints[1], programVariable.GetExternalAxes()[1]);
    EXPECT_EQ(externalJoints[2], programVariable.GetExternalAxes()[2]);
  }
...
TEST(App_PositionVariableTest, ConstructorBoth) {
  {
    ...
    std::array<double, 6> robotJoints{0};
    std::array<double, 3> externalJoints{0};
    App::DataTypes::PositionVariable programVariable(
        name, cartesianPos, robotJoints, externalJoints);
...
TEST(App_PositionVariableTest, GetSetRobotAxes) {
  ...
  for (size_t i = 0; i < 16; i++)
    EXPECT_EQ(cartesianExpect[i], programVariable.GetCartesian()[i]);
  for (size_t i = 0; i < programVariable.GetRobotAxes().size(); i++)
    EXPECT_EQ(newRobotJoints[i], programVariable.GetRobotAxes()[i]);
  EXPECT_EQ(externalJoints[0], programVariable.GetExternalAxes()[0]);
  EXPECT_EQ(externalJoints[1], programVariable.GetExternalAxes()[1]);
  EXPECT_EQ(externalJoints[2], programVariable.GetExternalAxes()[2]);
}
