...
TEST(App_MotionStateTest, ConstructorDefault) {
  ...
  // motion program
  EXPECT_EQ(App::DataTypes::MotionState::RunState::NOT_RUNNING,
            ms.motionProgram.runState);
  EXPECT_EQ(App::DataTypes::MotionState::ReplayMode::SINGLE,
            ms.motionProgram.replayMode);
  ...
  // logic program
  EXPECT_EQ(App::DataTypes::MotionState::RunState::NOT_RUNNING,
            ms.logicProgram.runState);
  EXPECT_EQ(App::DataTypes::MotionState::ReplayMode::SINGLE,
            ms.logicProgram.replayMode);
...
TEST(App_MotionStateTest, ConstructorGRPC) {
  {
    ...
    // motion program
    grpcState.mutable_motion_ipo()->set_runstate(
        robotcontrolapp::RunState::RUNNING);
    grpcState.mutable_motion_ipo()->set_replay_mode(
        robotcontrolapp::ReplayMode::STEP);
    ...
    // logic program
    grpcState.mutable_logic_ipo()->set_runstate(
        robotcontrolapp::RunState::RUNNING);
    grpcState.mutable_logic_ipo()->set_replay_mode(
        robotcontrolapp::ReplayMode::REPEAT);
    ...
    // move-to ipo
    grpcState.mutable_move_to_ipo()->set_runstate(
        robotcontrolapp::RunState::PAUSED);
    grpcState.mutable_move_to_ipo()->set_replay_mode(
        robotcontrolapp::ReplayMode::STEP);
    ...
    // motion program
    EXPECT_EQ(App::DataTypes::MotionState::RunState::RUNNING,
              ms.motionProgram.runState);
    EXPECT_EQ(App::DataTypes::MotionState::ReplayMode::STEP,
              ms.motionProgram.replayMode);
    ...
    EXPECT_EQ(App::DataTypes::MotionState::RunState::RUNNING,
              ms.logicProgram.runState);
    EXPECT_EQ(App::DataTypes::MotionState::ReplayMode::REPEAT,
              ms.logicProgram.replayMode);
    ...
    // move-to ipo
    EXPECT_EQ(App::DataTypes::MotionState::RunState::PAUSED,
              ms.moveTo.runState);
    EXPECT_EQ(App::DataTypes::MotionState::ReplayMode::STEP,
              ms.moveTo.replayMode);
  ...
  {
    robotcontrolapp::MotionState grpcState;
    grpcState.set_current_source(
        robotcontrolapp::MotionState_MotionSource_PLATFORM);

    // motion program
    grpcState.mutable_motion_ipo()->set_runstate(
        robotcontrolapp::RunState::PAUSED);
    grpcState.mutable_motion_ipo()->set_replay_mode(
        robotcontrolapp::ReplayMode::SINGLE);

    // logic program
    grpcState.mutable_logic_ipo()->set_runstate(
        robotcontrolapp::RunState::NOT_RUNNING);
    grpcState.mutable_logic_ipo()->set_replay_mode(
        robotcontrolapp::ReplayMode::STEP);
...

  {
    robotcontrolapp::MotionState grpcState;
    grpcState.set_current_source(
        robotcontrolapp::MotionState_MotionSource_POSITION_INTERFACE);
