...
TEST(App_RobotStateJointTest, ConstructorDefault) {
  ...
  EXPECT_EQ(App::DataTypes::RobotState::HardwareState::OKAY,
            joint.hardwareState);
  EXPECT_EQ(App::DataTypes::RobotState::ReferencingState::NOT_REFERENCED,
            joint.referencingState);
...
TEST(App_RobotStateJointTest, ConstructorGRPC) {
  {
    ...
    jointGrpc.set_state(
        robotcontrolapp::HardwareState::ERROR_ESTOP_LOW_VOLTAGE);
    jointGrpc.set_referencing_state(
        robotcontrolapp::ReferencingState::IS_REFERENCING);
    ...
    EXPECT_EQ(
        App::DataTypes::RobotState::HardwareState::ERROR_ESTOP_LOW_VOLTAGE,
        joint.hardwareState);
    EXPECT_EQ(App::DataTypes::RobotState::ReferencingState::IS_REFERENCING,
              joint.referencingState);
...

  {
    ...
    robotcontrolapp::HardwareState state =
        (robotcontrolapp::HardwareState)(robotcontrolapp::HardwareState::
                                             ERROR_MOTOR_NOT_ENABLED |
                                         robotcontrolapp::HardwareState::
                                             ERROR_DRIVER);
    jointGrpc.set_state(state);
    jointGrpc.set_referencing_state(
        robotcontrolapp::ReferencingState::NOT_REFERENCED);

    App::DataTypes::RobotState::Joint joint(jointGrpc);

    App::DataTypes::RobotState::HardwareState expectState =
        (App::DataTypes::RobotState::
             HardwareState)((int)App::DataTypes::RobotState::HardwareState::
                                ERROR_MOTOR_NOT_ENABLED |
                            (int)App::DataTypes::RobotState::HardwareState::
                                ERROR_DRIVER);
...

  {
    ...
    jointGrpc.set_referencing_state(
        robotcontrolapp::ReferencingState::IS_REFERENCED);

    App::DataTypes::RobotState::Joint joint(jointGrpc);

    EXPECT_EQ(App::DataTypes::RobotState::HardwareState::OKAY,
              joint.hardwareState);
    EXPECT_EQ(App::DataTypes::RobotState::ReferencingState::IS_REFERENCED,
              joint.referencingState);
  }
}
...
TEST(App_RobotStateTest, ConstructorDefault) {
  ...
  EXPECT_EQ(robotcontrolapp::KinematicState::KINEMATIC_NORMAL,
            robotState.kinematicState);
...
TEST(App_RobotStateTest, ConstructorGRPC) {
  {
    ...
    *robotStateGrpc.mutable_tcp() = tcp.ToGrpc();
    ...
    for (unsigned i = 0; i < 9; i++)
      robotStateGrpc.add_joints()->set_id(i + 1);

    for (int i = 0; i < 64; i++) {
      robotcontrolapp::DIn *din = robotStateGrpc.add_dins();
      din->set_id(i);
      din->set_state(i % 2 == 0 ? robotcontrolapp::DIOState::HIGH
                                : robotcontrolapp::DIOState::LOW);
      robotcontrolapp::DOut *dout = robotStateGrpc.add_douts();
      dout->set_id(i);
      dout->set_state(i % 2 != 0 ? robotcontrolapp::DIOState::HIGH
                                 : robotcontrolapp::DIOState::LOW);
    }
    ...
    for (int i = 0; i < 100; i++) {
      ...
      gsig->set_id(i);
      gsig->set_state(i % 2 == 0 ? robotcontrolapp::DIOState::HIGH
                                 : robotcontrolapp::DIOState::LOW);
    }

    robotStateGrpc.set_hardware_state_string("MNE_ENC");
    robotStateGrpc.set_kinematic_state(
        robotcontrolapp::KinematicState::KINEMATIC_ERROR_BRAKE_ACTIVE);
    ...
    robotStateGrpc.set_referencing_state(
        robotcontrolapp::ReferencingState::IS_REFERENCING);
    ...
    EXPECT_EQ(robotcontrolapp::KinematicState::KINEMATIC_ERROR_BRAKE_ACTIVE,
              robotState.kinematicState);
