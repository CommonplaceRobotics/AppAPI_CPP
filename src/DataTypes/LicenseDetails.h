/**
 * @brief The LicenseInfo contains info about the licensed features of the robot
 * control.
 * @author MAB
 */
#pragma once
...
public:
   ...
   */
  LicenseDetails(
      const robotcontrolapp::LicenseInfoResponse_LicenseDetails &info);
};
...
class LicenseInfo {
public:
  // Number of remaining seconds in the feature test duration or 0 if expired.
  // Within this duration some features can be tested without a license.
  ...
  /**
   * @brief Gets a feature from the features list. Throws std::invalid_argument
   * if the feature is not defined (use HasFeature()!).
   * @brief Feature
   */
