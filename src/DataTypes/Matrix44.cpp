...
Matrix44::Matrix44(const robotcontrolapp::Matrix44 &other) {
  if (other.data().size() != (int)m_data.size())
    throw std::runtime_error("could not initialize Matrix44, GRPC matrix had "
 ...
 */
void Matrix44::SetA(double a) { SetOrientation(a, GetB(), GetC()); }

/**
 * @brief Sets the B orientation
 * @param b B angle in degrees
 */
void Matrix44::SetB(double b) { SetOrientation(GetA(), b, GetC()); }

/**
 * @brief Sets the C orientation
 * @param c C angle in degrees
 */
void Matrix44::SetC(double c) { SetOrientation(GetA(), GetB(), c); }
