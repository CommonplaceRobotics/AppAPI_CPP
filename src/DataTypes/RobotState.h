/**
 * @brief The RobotState contains the most relevant, quickly changing state info
 * of the robot, e.g. its position and joint angles, IO, etc.
 * @author MAB
 */
...
/**
 * @brief This class contains most relevant info about the robot's state, e.g.
 * position, IO and errors
 */
class RobotState {
public:
  /**
   * @brief Position and orientation of the TCP in cartesian space (position in
   * mm)
   */
  Matrix44 tcp;
  ...
  enum class ReferencingState { NOT_REFERENCED, IS_REFERENCED, IS_REFERENCING };
  ...
  struct Joint {
    ...
    /**
     * @brief Target velocity in degrees/s, mm/s or user defined units per
     * second - only usable with external axes in velocity mode
     */
  ...
  /**
   * @brief Joint angles/positions in degrees, mm or user defined units. Indices
   * 0-5 are robot joints, 6-8 are external joints.
   */
   ...
   */
  robotcontrolapp::KinematicState kinematicState =
      robotcontrolapp::KinematicState::KINEMATIC_NORMAL;
  ...
  /**
   * @brief Combined current of all motors and DIO in mA (available for some
   * robots only)
   */
  float currentAll = 0;
