/**
 * @brief The SystemInfo contains information that usually does not change
 * (except for the cycle time statistics)
 * @author MAB
 */
#pragma once
...
/**
 * @brief This class contains system information that usually does not change
 * (except for the cycle time statistics)
 */
class SystemInfo {
public:
  ...
  int versionPatch = 0;
  // Robot control software version string, possibly with suffix, e.g.
  // "V14-003-1-RC1"
  std::string version;
  ...
  robotcontrolapp::SystemInfo_Voltage voltage =
      robotcontrolapp::SystemInfo_Voltage_Voltage24V;

  // System type of the robot control
  robotcontrolapp::SystemInfo::SystemType systemType =
      robotcontrolapp::SystemInfo_SystemType_Other;
  ...
  // Robot main loop cycle time (hardware IO, kinematics, program execution)
  // target in ms
  ...
  // Average workload in percent (how much of the available time is used vs.
  // waited)
  float workload = 0;
