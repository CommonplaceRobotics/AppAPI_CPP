...
public:
  ...
  /**
   * @brief Destructor
   * Note: this only exists since we need at least one virtual method for
...
public:
   ...
   */
  NumberVariable(const std::string &name, double value = 0)
      : ProgramVariable(name), m_value(value) {}
...
public:
   ...
   */
  PositionVariable(const std::string &name, const Matrix44 cartesian,
                   const std::array<double, EXTERNAL_AXES_COUNT> &externalAxes)
      : ProgramVariable(name), m_externalAxes(externalAxes),
   ...
   * @param externalAxes external axes
   */
  PositionVariable(const std::string &name,
                   const std::array<double, ROBOT_AXES_COUNT> &robotAxes,
                   const std::array<double, EXTERNAL_AXES_COUNT> &externalAxes)
      : ProgramVariable(name), m_robotAxes(robotAxes),
        m_externalAxes(externalAxes) {}
  ...
  PositionVariable(const std::string &name, const Matrix44 cartesian,
                   ...
                   const std::array<double, EXTERNAL_AXES_COUNT> &externalAxes)
      : ProgramVariable(name), m_robotAxes(robotAxes),
        m_externalAxes(externalAxes), m_cartesian(cartesian) {}

  /**
   * @brief Gets the robot axes
   * @return array of robot axes
   */
  const std::array<double, ROBOT_AXES_COUNT> &GetRobotAxes() const {
    return m_robotAxes;
  }
  /**
   ...
   */
  const std::array<double, EXTERNAL_AXES_COUNT> &GetExternalAxes() const {
    return m_externalAxes;
  }
  /**
   ...
   */
  void SetRobotAxes(const std::array<double, ROBOT_AXES_COUNT> &axes) {
    m_robotAxes = axes;
  }
  /**
   ...
   */
  void SetExternalAxes(std::array<double, EXTERNAL_AXES_COUNT> &axes) {
    m_externalAxes = axes;
  }
  /**
