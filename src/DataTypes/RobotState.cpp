...
RobotState::RobotState(const robotcontrolapp::RobotState &state) {
  ...
  digitalOutputs.resize(64);
  globalSignals.resize(100);
  for (size_t i = 0; (int)i < state.dins_size() && i < digitalInputs.size();
       i++) {
    digitalInputs[i] =
        state.dins((int)i).state() == robotcontrolapp::DIOState::HIGH;
  }
  for (size_t i = 0; (int)i < state.douts_size() && i < digitalOutputs.size();
       i++) {
    digitalOutputs[i] =
        state.douts((int)i).state() == robotcontrolapp::DIOState::HIGH;
  }
  for (size_t i = 0; (int)i < state.gsigs_size() && i < globalSignals.size();
       i++) {
    globalSignals[i] =
        state.gsigs((int)i).state() == robotcontrolapp::DIOState::HIGH;
  }
...
RobotState::Joint::Joint(const robotcontrolapp::Joint &joint) {
  ...
  hardwareState =
      static_cast<DataTypes::RobotState::HardwareState>(joint.state());
...
/**
 * @brief Checks whether all motors and IO modules are enabled. If false motion
 * is not possible.
 * @return true if enabled, false if disabled or hardware error
 */
bool RobotState::IsEnabled() const { return hardwareState == "NoError"; }
