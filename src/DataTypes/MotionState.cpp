 ...
 * @param state
 */
MotionState::InterpolatorState::InterpolatorState(
...
MotionState::MotionState(const robotcontrolapp::MotionState &state)
    : motionProgram(state.motion_ipo()), logicProgram(state.logic_ipo()),
      moveTo(state.move_to_ipo()) {
  ...
  if (state.has_request_successful())
    requestSuccessful = state.request_successful();
}
