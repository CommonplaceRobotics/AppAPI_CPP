...
Statistics::Statistics(const robotcontrolapp::StatisticsResponse &grpc) {
  ...
  for (int i = 0; i < (int)robotAxisDirectionChanges.size() &&
                  i < grpc.robot_axis_direction_changes_size();
       i++) {
    robotAxisDirectionChanges[i] = grpc.robot_axis_direction_changes(i);
  }
  for (int i = 0; i < (int)externalAxisDirectionChanges.size() &&
                  i < grpc.external_axis_direction_changes_size();
       i++) {
