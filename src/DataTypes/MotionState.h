/**
 * @brief The MotionState contains the state of the motion interpolators (i.e.
 * programs, move-to commands and the position interface)
 * @author MAB
 */
...
/**
 * @brief This class contains the state of the motion interpolators (which run
 * the robot programs)
 */
class MotionState {
public:
  /// Is a program running or paused?
  enum class RunState { NOT_RUNNING, PAUSED, RUNNING };
  /// Repeat the program?
  enum class ReplayMode { SINGLE, REPEAT, STEP };
  ...
  struct InterpolatorState {
    ...
    /**
     * @brief Index of the (sub-)program that is currently being executed: 0 is
     * the main program, higher numbers are sub-programs
     */
    int currentProgramIndex = 0;
    /**
     * @brief Number of loaded programs: 0 - no programs, 1 - only the main
     * program, higher values - the main and sub programs are loaded
     */
    int programCount = 0;
    /**
     * @brief Index of the current command that is being executed. 0 is the
     * first command in the current (sub-)program, -1 when not running.
     */
     ...
     */
    InterpolatorState(
        const robotcontrolapp::MotionState_InterpolatorState &state);
  };
  ...
  /**
   * @brief State of the Move-To interpolator (expect 0 or 1 program with only 1
   * command)
   */
  ...
  /**
   * @brief If this MotionState was sent in response to a request (specifically
   * program load, start and move-to starts) this value is set true if the
