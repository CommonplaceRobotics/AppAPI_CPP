/**
 * @brief This file defines an abstract app client. It provides a simplified API
 * to the GRPC interface. Derive your app class from AppClient and implement the
 * abstract methods.
 * @author MAB
 */
...
/**
 * @brief This exception is thrown when trying to call a RPC while the app is
 * not connected
 */
...
public:
   ...
   */
  AppClient(const std::string &appName,
            const std::string &target = std::string(TARGET_LOCALHOST));
  /**
  ...
  /**
   * @brief Disconnects the app. Make sure to disconnect in the derived
   * destructor!
   */
  void Disconnect();
  ...
  /**
   * @brief Announces to the robot control that the app function call failed.
   * This will abort the program with an error message. This is supported from
   * V14-003
   * @param callId function call ID from the function call request
  ...
  // =========================================================================
  /**
   * @brief Gets the given program variable, throws exception on error, e.g. if
   * the variable does not exist
   * @param variableName name of the variable
   * @return NumberVariable or PositionVariable
   */
  std::shared_ptr<DataTypes::ProgramVariable>
  GetProgramVariable(const std::string &variableName);

  /**
   * @brief Gets the given number variable, throws on error, e.g. if the
   * variable does not exist or is of a different type
   * @param variableName name of the variable
   * @return number variable
   */
  std::shared_ptr<DataTypes::NumberVariable>
  GetNumberVariable(const std::string &variableName);
  ...
  GetPositionVariable(const std::string &variableName);
  ...
  GetProgramVariables(const std::unordered_set<std::string> &variableNames);
  ...
  /**
   * @brief Sets a position variable with joint angles. The robot control will
   * try to convert these to cartesian.
   * @param name name of the variable
   ...
   */
  void SetPositionVariable(const std::string &name, double a1 = 0,
                           double a2 = 0, double a3 = 0, double a4 = 0,
                           double a5 = 0, double a6 = 0, double e1 = 0,
                           double e2 = 0, double e3 = 0);
  /**
   * @brief Sets a position variable with a cartesian position. The robot
   * control will try to convert this to joint angles.
   * @param name name of the variable
   ...
   * @param e3 position of external axis 3 in degrees, mm or user defined units
   */
  void SetPositionVariable(const std::string &name,
                           DataTypes::Matrix44 cartesianPosition, double e1 = 0,
                           double e2 = 0, double e3 = 0);
   ...
   */
  void SetPositionVariable(const std::string &name,
                           DataTypes::Matrix44 cartesianPosition, double a1,
                           double a2, double a3, double a4, double a5,
                           double a6, double e1 = 0, double e2 = 0,
                           double e3 = 0);
  ...
  // =========================================================================
  /**
   * @brief Starts referencing all joints. Note that axes will enter position
   ...
   * reference before calling the program. Note that axes will enter position
   * lag error when successfully referenced, this is no issue, simply re-enable
   * the motors.
   */
  ...
  /**
   * @brief Starts referencing robot and external joints without delay. Note
   * that axes will enter position lag error when successfully referenced, this
   * is no issue, simply re-enable the motors.
  ...
  /**
   * @brief Is called when the robot state is updated (usually each 10 or 20ms).
   * Override this method, start the stream by calling StartRobotStateStream().
  ...
  /**
   * @brief Sets the states of the global signals. This bundles all changes in
   * one request.
   ...
   * @return motion state, check request_successful and
   * motionProgram.mainProgram for success
   */
   ...
   */
  // DataTypes::MotionState StartMotionProgramAt(unsigned commandIdx = 0, const
  ...
  MoveToJoint(float velocityPercent, float acceleration = 40, double a1 = 0,
              double a2 = 0, double a3 = 0, double a4 = 0, double a5 = 0,
   ...
   */
  DataTypes::MotionState
  MoveToJointRelative(float velocityPercent, float acceleration = 40,
   ...
   * @return motion state
   */
  DataTypes::MotionState MoveToLinear(float velocityMms,
                                      float acceleration = 40, double x = 0,
                                      double y = 0, double z = 0, double a = 0,
                                      double b = 0, double c = 0, double e1 = 0,
                                      double e2 = 0, double e3 = 0,
   ...
   * @return motion state
   */
  DataTypes::MotionState MoveToLinearRelativeBase(
      float velocityMms, float acceleration = 40, double x = 0, double y = 0,
      double z = 0, double a = 0, double b = 0, double c = 0, double e1 = 0,
      double e2 = 0, double e3 = 0, const std::string &frame = "");
  /**
   * @brief Starts a linear motion to the given position
   * @param velocityMms velocity in mm/s
   * @param acceleration acceleration in percent, 0.0..100.0, negative values
   ...
   */
  DataTypes::MotionState
  MoveToLinearRelativeTool(float velocityMms, float acceleration = 40,
  ...
  /**
   * @brief Returns true if the robot moves automatically. This does not
   * indicate other motion types, like jog motion!
   ...
   * timeout is exceeded. Default: Effectively infinitely.
   ...
   */
  bool WaitMotionDone(
      std::chrono::steady_clock::duration timeout = std::chrono::hours(9999999),
  ...
  /**
   * @brief Checks whether the given feature is licensed via the robot control
   * and is not expired.
   * @param id feature ID
   ...
   * @param initialJoints 6 robot joints and 3 external joints. These are used
   * to derive the initial joint configuration, e.g. whether the elbow points
   * left or right. Set them to 0 if not relevant.
   ...
   */
  bool TranslateCartToJoint(double x, double y, double z, double a, double b,
                            double c,
   ...
   */
  bool TranslateJointToCart(const std::array<double, 9> &joints, double &x,
   ...
   * @brief Translates joint positions to a cartesian position
   * @param joints joint positions to translate
   * @param tcp the matrix defining position and orientation of the TCP is
   * written here
   * @param resultState the result state is written here. 0 on success, other
   * value on error
   * @return true on success
   */
  bool TranslateJointToCart(const std::array<double, 9> &joints,
                            DataTypes::Matrix44 &tcp,
                            robotcontrolapp::KinematicState &resultState);
   ...
   * @param sourceFile local source file with path relative to the app's
   * directory
   ...
   * @return true on success
   */
  bool UploadFile(const std::string &sourceFile, const std::string targetFile,
                  std::string &error);
  /**
   ...
   * @return true on success
   */
  bool UploadFile(uint8_t *data, size_t length, const std::string targetFile,
                  std::string &error);
  /**
   ...
   * @return true on success
   */
  bool DownloadFile(const std::string &sourceFile, const std::string targetFile,
                    std::string &error);
  /**
   * @brief Downloads a file from the robot control to memory
   * @param sourceFile source file on the robot control, relative to the Data
   * directory
   * @param data file data is written to this vector. It will be cleared and
   * it's size changed
   * @param error if an error occurs the reason is written here
   * @return true on success
   */
  bool DownloadFile(const std::string &sourceFile, std::vector<uint8_t> &data,
                    std::string &error);
   ...
   * with a call of UiUpdateHandler() if the element exists and if it was
  ...
  /**
   * @brief Queues a request of the state of a UI element. The robot control
   * will respond with a call of UiUpdateHandler() if the element exists and if
   ...
   * respond with a call of UiUpdateHandler() if the element exists and if it
   * was changed after start up
   * @param elementNames set of UI element names
   */
  void
  RequestUIElementStates(const std::unordered_set<std::string> &elementNames);
  /**
   ...
   * @param elementNames set of UI element names
   */
  void QueueRequestUIElementStates(
   ...
   */
  void
  QueueSetUIVisibility(const std::unordered_map<std::string, bool> &elements);
   ...
   */
  void SetDropDownState(const std::string &elementName,
   ...
   * @param selectedValue selected value, must be a defined selectable entry
   */
  void QueueSetDropDownState(const std::string &elementName,
                             const std::string &selectedValue);
  ...
  /**
   * @brief Queues setting the selected value and the list of selectable values
   * of a drop down box
   * @param elementName ID of the UI element
   * @param selectedValue selected value, must be a defined selectable entry
   * @param selectableEntries selectable values
  ...
  /**
   * @brief Queues seting the number value of a number box, text box, label,
   * etc.
   * @param elementName ID of the UI element
   * @param value number
   */
   ...
   * @param uiHeight height of the image UI element (does not need to be equal
   * to the image height, image will be scaled) - currently not used yet
   * @param imageData binary image data
   * @param imageDataLength length of the image data. All shown images combined
   * must be less than 290kB, otherwise the UI may fail to load after reconnect!
   * @param encoding image format
   */
  void SetImage(const std::string &elementName, unsigned uiWidth,
                unsigned uiHeight, const uint8_t *imageData,
                size_t imageDataLength,
                robotcontrolapp::ImageState::ImageData::ImageEncoding encoding =
                    robotcontrolapp::ImageState_ImageData_ImageEncoding_JPEG);
   ...
   * @param uiHeight height of the image UI element (does not need to be equal
   * to the image height, image will be scaled)
   * @param imageFile file name and path of the image file to load. All shown
   * images combined must be less than 290kB, otherwise the UI may fail to load
   * after reconnect!
   */
  void SetImage(const std::string &elementName, unsigned uiWidth,
   ...
   */
  void
  QueueSetImage(const std::string &elementName, unsigned uiWidth,
                unsigned uiHeight, uint8_t *imageData, size_t imageDataLength,
                robotcontrolapp::ImageState::ImageData::ImageEncoding encoding =
                    robotcontrolapp::ImageState_ImageData_ImageEncoding_JPEG);

  /**
   * @brief Shows an info dialog window to the user.
   * Note: If iRC is not connected or older than V14-004 the dialog will never
   * be shown. Currently there is no way for the app to find out whether this is
   * the case. than V14-004 only error messages are shown.
   ...
   * Note: If iRC is not connected the dialog will never be shown. Currently
   * there is no way for the app to find out whether this is the case. than
   * V14-004 only error messages are shown.
   * @param message The message to be displayed
   * @param title The dialog title
   */
...
protected:
  /**
   * @brief Gets called on remote app function calls received from the robot
   * control
   * @param function app function data
   */
  virtual void
  AppFunctionHandler(const robotcontrolapp::AppFunction &function) = 0;
  /**
   * @brief Gets called on remote UI update requests received from the robot
   * control
   * @param updates updated UI elements. Key is the element name, value contains
   * the changes.
   */
  virtual void UiUpdateHandler(
      const std::map<std::string, const robotcontrolapp::AppUIElement *>
          &updates) = 0;

  /**
   * @brief GRPC client stub: This is the generated GRPC client interface.
   * It communicates with the server and provides access to actions, streams
   * etc.
   */
...
private:
  ...
  std::shared_ptr<grpc::ClientReaderWriter<robotcontrolapp::AppAction,
      ...
      m_grpcStream;
  /**
   * @brief GRPC context. This context is only for the grpcStream, create
   * temporary contexts for other requests!
   */
   ...
   */
  DataTypes::MotionState
  SetMotionProgramReplayMode(robotcontrolapp::ReplayMode replayMode);
  /**
   ...
   */
  DataTypes::MotionState
  SetMotionProgramRunState(robotcontrolapp::RunState runState);

  /**
   * @brief Shows a dialog window to the user.
   * Note: If iRC is not connected the dialog will never be shown. Currently
   * there is no way for the app to find out whether this is the case. If iRC is
   * older than V14-004 only error messages are shown.
   * @param message The message to be displayed
   * @param title The dialog title
